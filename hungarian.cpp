...
static struct PyModuleDef hungarianmodule = {
    PyModuleDef_HEAD_INIT,
    "hungarian",   /* name of module */
...
PyMODINIT_FUNC 
PyInit_hungarian(void)
{
    //(void) Py_InitModule("hungarian", HungarianMethods);
    PyObject *m;
    m = PyModule_Create(&hungarianmodule);
    if (m == NULL)
        return NULL;
...
{
    PyImport_AppendInittab("hungarian", PyInit_hungarian);

    /* Pass argv[0] to the Python interpreter */
    wchar_t progname[50 + 1];
    ...
    /* Add a static module */
    PyImport_ImportModule("hungarian");
